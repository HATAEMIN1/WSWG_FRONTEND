...
function ReviewAdd(props) {
    ...
    useEffect(() => {
        async function restaurantView() {
            const res = await axiosInstance.get(`/restaurants/${cateId}/${rtId}`);
                ...
                <div className="w-full min-h-[120px] flex justify-between bg-[#F8F8F8] rounded-lg overflow-hidden border items-center">
                    <div className="w-[100px] overflow-hidden border-r-[1px] p-2">
                        {restaurantData.length > 0 && (
                            <img src={restaurantData[0].image[0]} alt="" className="block" />
                        )}
                    </div>
                    ...
                    <div className="flex gap-2 pt-2">
                        {hashtag.length > 0 &&
                            hashtag.map((hashTag, index) => (
                                <div key={hashTag} className="flex relative items-center gap-2">
                ...
                <div className="mb-10">
                    <div>
                        ...
                        <div className="w-full relative bg-[#f5f5f5] h-[150px]">
                            ...
                            <FileUpload images={text.images} onImageChange={handleImage} />
                    ...
                    <div className="text-md flex justify-center mb-5 text-red-600">
                        *타인을 비방하거나 불건전한 내용을 등록 시 삭제 될 수 있습니다.
                    </div>
                </div>
                <div className="mb-32">
                    <ButtonWrap>
                        <Button basicButton={true}>등록</Button>
                        <ButtonCencel onClick={handleCancelClick}>취소</ButtonCencel>
                    </ButtonWrap>
                </div>
            </form>
